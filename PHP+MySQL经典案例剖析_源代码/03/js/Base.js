...
function CheckPassword(value)
{
	...
	document.getElementById("Pwd").className = 'msg';
	document.getElementById("Pwd").innerHTML = 'OK';
	return true;
}
/**
 * 功能：检验重复密码
...
function CheckConfirm(value)
{
	if(value != document.register.password.value)					//判断重复密码是否与密码相符
	{
		...
		document.getElementById("Confirm_Pwd").innerHTML = '重复密码和密码不一致!';
		return false;
	}
	document.getElementById("Confirm_Pwd").className = 'msg';
	document.getElementById("Confirm_Pwd").innerHTML = 'OK';
	return true;
}
/**
 * 功能：检验昵称
...
function CheckNickName(value)
{
	...
	if(!checkByteLength(value,4,20))							//判断昵称长度是否OK
	{
		document.getElementById("Nick").className = 'error';
		document.getElementById("Nick").innerHTML = '昵称长度不OK！昵称长度为4－20位。';
		return false;
	}
	document.getElementById("Nick").className = 'msg';
	document.getElementById("Nick").innerHTML = 'OK';
	return true;
}
/**
 * 功能：检验表单的各项是否OK
...
function CheckForm()
{
	...
	if(!CheckNickName(document.register.NickName.value))			//判断昵称是否OK
	{
		return false;
	}
	return true;
...
{
	var flag = false;
	for(j= 0 ; j < parent.document.frm_write.name.options.length ; j++)		//循环列表每一个选项
	{
		if(parent.document.frm_write.name.options[j].value == value)		//判断该选项是否存在
			flag = true;
	}
	if(!flag)												//如果该选项不存在
	{
		SelectOption = new Option(name,value);					//生成新选项
		parent.document.frm_write.name.options[parent.document.frm_write.name.options.length] = SelectOption ;
	}
}