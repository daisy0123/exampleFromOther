...
abstract class Zend_Db_Adapter_Abstract
{
    ...
    public function insert($table, $bind)
    {
        ...
        $strFields = implode(',', $cols);  // auto insert F_ID as $table.'_SEQ.NEXTVAL'
        $strValues = ":" . implode(', :', $cols);
    ...
    /**
     * Insert/Update table rows with specified data based on whether a F_ID is defined or not
     *
     * @param string $table
     * @param array $data
     * @return integer effected data number
     */
    public function save($table, $data)
    {
         if (isset($data['F_ID'])){
             $Key = $data['F_ID'];
             $temp = array_diff_key($data,array('F_ID'=>$Key));
             return $this->update($table,$temp,'F_ID = ' . $Key);
         }  else {
    ...
    {
        $result = $this->query($sql, $bind);
		//return $result->fetchAll(Zend_Db::FETCH_ARRAY);
