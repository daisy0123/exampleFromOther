...
class Zend_Db_Adapter_Oracle extends Zend_Db_Adapter_Abstract
{
    ...
    public function lastInsertId($name = null)
    {
		...
		return $data['CURRVAL'][0]; //we can't fail here, right? if the sequence doesn't exist we should fail earlier.
    }
    ...
    public function describeTable($table)
    {
		...
		$sql = "SELECT COLUMN_NAME, DATA_TYPE, NULLABLE FROM DBA_TAB_COLUMNS "
				."WHERE TABLE_NAME = UPPER('{$table}') AND OWNER = UPPER('{$UserName}')";

		$result = $this->fetchAll($sql);
		
		foreach ($result  as $row) {
			$descr[$row['COLUMN_NAME']] = array(
				'name'		=> $row['COLUMN_NAME'],
				'notnull'	=> (bool) ($row['NULLABLE'] === 'Y'),
				'type'		=> $row['DATA_TYPE']
			);
		}
		return $descr;
    ...
    public function limit($sql, $count, $offset)
    {
        ...
        */
		if ($count != 0 && $offset != 0)
			$limit = "WHERE zrownum BETWEEN ".$offset." AND ".($offset+$count-1);
		else
			$limit = "";
