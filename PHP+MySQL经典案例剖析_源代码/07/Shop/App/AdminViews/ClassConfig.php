...
{*if $info[F_CLASS_IMG_DEFAULT]*}
 Checked
{*/if*} />默认 </td>
		    <td>宽度
              <input name="img_width" type="text" id="img_width" value="{*$info[F_CLASS_IMG_WIDTH]*}" size="10"
 {*if $info[F_CLASS_IMG_DEFAULT]*}
 disabled
{*/if*} />
像素</td>
		    <td>高度
              <input name="img_height" type="text" id="img_height" value="{*$info[F_CLASS_IMG_HEIGHT]*}" size="10"
{*if $info[F_CLASS_IMG_DEFAULT]*}
 disabled
{*/if*} />
像素</td>
...
{*if $info[F_CLASS_SMALL_IMG_DEFAULT]*}
 checked
{*/if*} />
			  默认 </td>
			<td>宽度 
			  <input name="small_width" type="text" id="small_width" value="{*$info[F_CLASS_SMALL_IMG_WIDTH]*}" size="10"
{*if $info[F_CLASS_SMALL_IMG_DEFAULT]*}
 disabled
{*/if*} />像素 </td>
			<td>高度 
			  <input name="small_height" type="text" id="small_height" value="{*$info[F_CLASS_SMALL_IMG_HEIGHT]*}" size="10"
{*if $info[F_CLASS_SMALL_IMG_DEFAULT]*}
 disabled
{*/if*} /> 像素</td>
		  </tr>
...
{*if $info[F_CLASS_BIG_IMG_DEFAULT]*}
 checked
{*/if*} />默认 </td>
			<td>宽度 
			  <input name="big_width" type="text" id="big_width" value="{*$info[F_CLASS_BIG_IMG_WIDTH]*}" size="10"
{*if $info[F_CLASS_BIG_IMG_DEFAULT]*}
 disabled
{*/if*} />像素 </td>
			<td>高度 
			  <input name="big_height" type="text" id="big_height" value="{*$info[F_CLASS_BIG_IMG_HEIGHT]*}" size="10"
{*if $info[F_CLASS_BIG_IMG_DEFAULT]*}
 disabled
{*/if*}>像素</td>
		  </tr>
...
{*if $info[F_CLASS_PAGESIZE_DEFAULT]*}
 checked
{*/if*} />默认 </td>
			<td height="23"> 每页 
			  <input name="pagesize" type="text" id="pagesize" value="{*$info[F_CLASS_PAGESIZE]*}" size="10" 
{*if $info[F_CLASS_PAGESIZE_DEFAULT]*}
 disabled
{*/if*} />个 </td>
			<td height="23">&nbsp;</td>
...
{*if $info[F_CLASS_PERLINE_DEFAULT]*}
 checked
{*/if*} />默认 </td>
			<td height="23"> 每行 
			  <input name="perline" type="text" id="perline" value="{*$info[F_CLASS_PERLINE]*}" size="10"
{*if $info[F_CLASS_PERLINE_DEFAULT]*}
 disabled
{*/if*} />个 </td>
			<td height="23">&nbsp;</td>
