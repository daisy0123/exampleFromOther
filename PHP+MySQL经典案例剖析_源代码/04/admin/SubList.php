...
if($_SERVER['REQUEST_METHOD'] == 'POST')						//判断是否是提交操作
{
	if($_POST['SelId'])											//判断是否选择了试题
	{
		...
		{
			$Data->delData($del_id,"EE_SUBJECTIVE_INFO");
		}
	}
}
...
<tr>
<td class=caption colSpan=2>主 观 题 管 理</td>
</tr>
<tr>
...
<td colSpan=2><table width="100%" border=0>
  <tr>
    <th width=34><input id=allbox onclick=CA(); type=checkbox value=1 
name=allbox></th>
    <th width=296>标题</th>
    <th width=397>参考答案</th>
    <th width=62>管理</th>
  </tr>
  <?php
if($List)														//判断是否有试题资料
...
?>
  <tr bgColor=#ffffff>
    <td align=middle><input type=checkbox value='<?php echo $value[F_ID]?>' name=SelId[]>
    </td>
    <td><?php echo $value[F_SUBJECTIVE_NAME]?></td>
    <td align=left><?php echo $value[F_SUBJECTIVE_ANSWER]?></td>
    <td align=middle><a href="SubAdd.php?DataId=<?php echo $id?>&Id=<?php echo $value[F_ID]?>">[编辑]</a></td>
  </tr>
  <?php
}
}
...
<input type="button" name="Submit2" value=" 添 加 试 题 " onClick="javascript:window.location='SubAdd.php?DataId=<?php echo $id?>'">
<input type="button" name="Submit" value="设置试题顺序" onClick="javascript:window.location='SubOrder.php?Id=<?php echo $id?>'"></th>
</tr></table>
</form>
